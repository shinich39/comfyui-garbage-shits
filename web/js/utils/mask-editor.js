...
function initMaskEditor() {
  ...
  const widget = self.addDOMWidget("maskeditor", "", container, {
    serialize: false,
  ...
  ;(() => {
    if (self.widgets.find(e => e.name === "Sketch")) {
      return;
    }
    const w = self.addWidget("toggle", "Sketch", false, () => {}, {
      serialize: false,
    });
  ...
  ;(() => {
    if (self.widgets.find(e => e.name === "Clear")) {
      return;
    }

    const w = self.addWidget("button", "Clear", null, () => {}, {
      serialize: false,
    });
  ...
  // node keydown event
  const onKeyDown = self.onKeyDown;
...
function handleWheelEvent(self, e) {
  ...
  let factor = (e.shiftKey || e.ctrlKey || e.altKey || e.metaKey) ? 0.5 : 3;
...
async function saveEvent() {
  ...
  formData.append('path', node.$$shits.selectedImage.origPath);

  const response = await api.fetchApi('/shinich39/comfyui-garbage-shits/edit-image', {
    method: 'POST',
    body: formData
  });
  ...
  // set to loaded image
  node.$$shits.selectedImage.drawName = data.draw_name;
  node.$$shits.selectedImage.drawPath = data.draw_path;
  node.$$shits.selectedImage.maskName = data.mask_name;
  node.$$shits.selectedImage.maskPath = data.mask_path;
}

async function clearEvent() {
  const node = this.node;
  if (!node || !node.$$shits || !node.$$shits.selectedImage) {
    return;
  }

  await api.fetchApi('/shinich39/comfyui-garbage-shits/clear-image', {
    method: 'POST',
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      path: node.$$shits.selectedImage.origPath,
    }),
  });

  // set to loaded image
  node.$$shits.selectedImage.maskName = null;
  node.$$shits.selectedImage.maskPath = null;
  node.$$shits.selectedImage.drawName = null;
  node.$$shits.selectedImage.drawPath = null;
  ...
  // this.origImg.src = getImageURL(node.$$shits.selectedImage.origPath);
}
