...
async function getCkptWorkflows() {
  const response = await api.fetchApi(`/shinich39/comfyui-garbage-shits/get-ckpt-workflows`, {
    method: "GET",
...
function createCkptContent(ckpt, meta) {
  let str = "";
...
export default {
	name: `shinich39.GarbageShits.Civitai`,
  setup() {
    getCkptWorkflows().then(({ checkpoints }) => {
      ckptMap = checkpoints || {};
      console.log("[comfyui-garbage-shits] Civitai checkpoints loaded successfully");
    });
  },
